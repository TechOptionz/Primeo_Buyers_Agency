export type Step = { n: string; title: string; text: string; ph: string };
export type ServiceCard = { tag: string; title: string; text: string; foot: string; ph: string };
export type Column = { eyebrow: string; title: string; text: string; points: string[]; cta: string; ph: string };
...
export type Service = {
  ...
  cards: ServiceCard[];
  columns: Column[];
  quote: { text: string; name: string; meta: string; ph: string };
  ctaButton: string;
};
...
export const ARTICLES = [
  { id: 'one', cat: 'Market update', date: 'Sep 2026', title: 'Brisbane spring market: what buyers should expect', excerpt: 'Stock levels, clearance rates and where the competition is heading this quarter.', placeholder: 'Photo: Brisbane skyline from Kangaroo Point' },
  { id: 'two', cat: 'Buying', date: 'Aug 2026', title: 'Off-market properties, explained', excerpt: 'How pre-market and off-market deals actually happen and how to access them.', placeholder: 'Photo: front door detail' },
  { id: 'three', cat: 'Investing', date: 'Aug 2026', title: 'Growth corridors worth watching in 2027', excerpt: 'Infrastructure, land supply and the suburbs where the evidence stacks up.', placeholder: 'Photo: aerial of a new estate' },
];

export const PROPERTIES = [
  { id: 'a', status: 'For sale', price: '$1,850,000', address: '42 Moray Street, New Farm', type: 'House', beds: 4, baths: 3, cars: 2, agent: 'Jordan Reid · 0400 000 000', placeholder: 'Photo: renovated Queenslander exterior' },
  { id: 'b', status: 'Secured off-market', price: '$1,420,000', address: '12 Latrobe Terrace, Paddington', type: 'House', beds: 4, baths: 2, cars: 2, agent: 'Secured for a PRIMEO buyer', placeholder: 'Photo: character home with verandah' },
  { id: 'c', status: 'For lease', price: '$1,150 per week', address: '8/21 Oxlade Drive, New Farm', type: 'Apartment', beds: 3, baths: 2, cars: 1, agent: 'Property management', placeholder: 'Photo: riverfront apartment balcony' },
];

export const JOURNEY: Step[] = [
  { n: '01', title: 'Discover', text: 'A strategy session to define budget, location, lifestyle and growth priorities.', ph: 'Photo: strategy meeting over coffee' },
  { n: '02', title: 'Search', text: 'On-market, off-market and pre-market sourcing, shortlisted against your brief.', ph: 'Photo: Brisbane suburb streetscape' },
  { n: '03', title: 'Inspect', text: 'Walk-throughs, building and pest, comparable sales and contract review.', ph: 'Photo: inspecting a kitchen' },
  { n: '04', title: 'Negotiate', text: 'Private treaty or auction, run with a clear walk-away number.', ph: 'Photo: auction paddle raised' },
  { n: '05', title: 'Secure', text: 'Contract to settlement, handled, then keys in hand.', ph: 'Photo: keys at the front door' },
];
...
const withPh = <T extends object>(items: T[], phs: string[]) => items.map((x, i) => ({ ...x, ph: phs[i] || 'Photo' }));
...
export const SERVICES: Record<string, Service> = {
  buyers: {
    ...
    columns: [],
    quote: { text: 'They found us a home that was never advertised and negotiated $60k under the guide. Calm the whole way through.', name: 'Sarah & Tom Nguyen', meta: 'Bought in New Farm', ph: 'Photo: family at their new front door' },
  ...
  selling: {
    ...
    columns: [],
    quote: { text: 'The appraisal was honest, the campaign plan was clear, and we sold in three weeks above reserve.', name: 'Michael Andersen', meta: 'Sold in Paddington', ph: 'Photo: sold sticker on signboard' },
  ...
  rentals: {
    ...
    quote: { text: 'Statements arrive on time, repairs are handled fast and I always know who to call.', name: 'Grace Whitford', meta: 'Landlord, Bulimba', ph: 'Photo: landlord and property manager at apartment' },
  ...
  commercial: {
    ...
    columns: [],
    quote: { text: 'Our commercial purchase had a dozen moving parts. PRIMEO kept every one of them on track.', name: 'Priya Raman', meta: 'Commercial investor, Fortitude Valley', ph: 'Photo: investor in office foyer' },
  ...
  land: {
    ...
    columns: [],
    quote: { text: 'They told us which estates to avoid and why. We bought where the numbers made sense.', name: 'Liam & Aisha Carter', meta: 'First-home buyers, Moreton Bay', ph: 'Photo: couple at new home handover' },
